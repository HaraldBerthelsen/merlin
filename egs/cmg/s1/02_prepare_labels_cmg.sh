...
if test "$#" -ne 3; then
    echo "################################"
    echo "Usage:"
    echo "./02_prepare_labels_cmg.sh <path_to_wav_dir> <path_to_labels_dir> <demo|full>"
    echo ""
...
if [ "$copy" = true ]; then
	...
	head -60 $duration_data_dir/$FileIDList > /tmp/tmfilelist
	mv /tmp/tmfilelist $duration_data_dir/$FileIDList
	head -60 $acoustic_data_dir/$FileIDList > /tmp/tmfilelist
	mv /tmp/tmfilelist $acoustic_data_dir/$FileIDList
    fi
