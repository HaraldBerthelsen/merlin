...
def addProm(txtfile):
    ...
    promfile = os.path.join(indir,basename+".promtokens")
    ...
    texttokens = []
    promvalues = []
    for promline in promlines:
        print("PROMLINE: %s" % promline.strip())
        if promline.strip() != "":
            line = promline.strip().split()
            texttokens.append(line[0])
            promvalues.extend(line[1:])
